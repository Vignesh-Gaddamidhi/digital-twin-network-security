...
def run_twin_core_v01_test():
    ...
    dev3 = DeviceEntity(
        ...
        vulnerabilities=[
            VulnerabilityEntity(
                vuln_id="VULN-001", cve_id="CVE-2023-38408", name="OpenSSH PKCS#11 Forwarding RCE",
                severity="CRITICAL", cvss_score=9.8, affected_service="openssh", port=22, status="OPEN"
            )
        ],
        criticality=8.5
    )
    dev4 = DeviceEntity(
        id="DEV-004", hostname="rtr-gw-01", type="ROUTER", role="PERIMETER_GATEWAY",
        interfaces=[
        ...
        open_ports=[53, 67], services=["dnsmasq", "nat"], criticality=9.5
    )
    dev5 = DeviceEntity(
        id="DEV-005", hostname="sw-core-01", type="SWITCH", role="CORE_SWITCH",
        interfaces=[DeviceInterface(interface_id="vlan1", ip_address="0.0.0.0", mac_address="00:50:56:FE:01:FE", subnet="192.168.1.0/24")],
        open_ports=[], criticality=9.0
    )
    dev6 = DeviceEntity(
        id="DEV-006", hostname="fw-edge-01", type="FIREWALL", role="PERIMETER_INSPECTION",
        interfaces=[
        ...
        open_ports=[], services=["stateful-dpi", "packet-filter"], criticality=9.5
    )
